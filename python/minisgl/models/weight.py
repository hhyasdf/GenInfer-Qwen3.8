...
def load_weight(model_path: str, device: torch.device) -> Iterator[Tuple[str, torch.Tensor]]:
    ...
    For GGUF directories, dispatches to the GGUF weight loader (raw quantized
    bytes, no sharding/merging — the Q6_K kernels handle dequantization).
    """
    from .config import ModelConfig
    from .gguf_weight import is_gguf_model_path, load_gguf_weight

    if is_gguf_model_path(model_path):
        yield from load_gguf_weight(model_path, device)
        return
