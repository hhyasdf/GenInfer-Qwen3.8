...
async def benchmark_one(
    ...
    *,
    ...
    vendor_ext: bool = True,
) -> RawResult:
    if isinstance(pbar, bool):
        pbar = make_console(1, output_length, use_pbar=pbar)
    with pbar.inflight(1):
        # vendor_ext is this engine's extra_body (ignore_eos, top_k, and an
        # input-length override). The shared ruler (--base-url, used for both
        # the origin server and this engine) sends a plain OpenAI request.
        kwargs: Dict[str, Any] = {}
        if vendor_ext:
